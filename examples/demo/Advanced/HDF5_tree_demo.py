...
hierarchy of an HDF5 file.  This only picks out arrays and groups, but could
...
Note that a PyTables can't read HDF5 files created with h5py,
but h5py can read HDF5 files created with PyTables.
"""
...
def _get_sub_arrays(group, h5file):
    ...
    for array in h5file.iter_nodes(group, classname='Array'):
        a = Hdf5ArrayNode(name=array._v_name,
...
def main():
    import sys
    filename = os.path.join(ROOT, 'test_fixed.h5')  # 'test_table_no_dc.h5') #
    if len(sys.argv) > 1:
        filename = sys.argv[1]
